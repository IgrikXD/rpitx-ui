...
**Chirp ([`pichirp`](./src/pichirp/)) - rewritten.** The original implementation used `atof` for argument parsing (_which silently returns 0 on invalid input_), ran non-async-signal-safe `fprintf` from a handler installed for all 64 signals, mutated a plain `bool running` flag from that handler (_data race_), accepted bandwidths above the Nyquist limit without diagnosis, and contained dead code. The new version uses `std::atomic<int>` for the stop flag with the handler restricted to `SIGTERM` / `SIGINT`, uses shared CLI parser for input validation, adds an explicit Nyquist check, and switches phase accumulation to `double` with `std::numbers::pi_v<double>` for full precision on long sweeps.
...
[rpitx-ui-64-bit-build-badge]: https://img.shields.io/github/actions/workflow/status/IgrikXD/rpitx-ui/rpitx-ui-64-bit-build.yml?&longCache=true&style=for-the-badge&label=rpitx-ui%2064-bit%20build&logo=raspberry-pi
[rpitx-ui-32-bit-build-badge]: https://img.shields.io/github/actions/workflow/status/IgrikXD/rpitx-ui/rpitx-ui-32-bit-build.yml?&longCache=true&style=for-the-badge&label=rpitx-ui%2032-bit%20build&logo=raspberry-pi
