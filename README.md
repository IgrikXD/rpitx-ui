...
The narrow-band FM transmitter has been rewritten from the legacy `csdr`/`rpitx` shell pipeline into a standalone [`pinfm`](./src/pinfm/) binary. The original [`testnfm.sh`](./scripts/testnfm.sh) path converted PCM audio through several external `csdr` stages and fed `rpitx` in raw RF mode, with no built-in deviation presets, AGC, or explicit audio-bandwidth guard. The new version expects 16-bit PCM mono audio at 48 kHz from stdin, skips an initial WAV header when present, and drives `librpitx::ngfmdmasync` directly with per-sample frequency-deviation values. Its dedicated [`NfmProcessor`](./src/pinfm/nfm_processor.h) chain applies a 30 Hz HPF, a cascaded 4th-order 3000 Hz Butterworth LPF, scalar AGC, and a hard clamp before scaling to the selected peak deviation. [`easytest.sh`](./easytest.sh) now asks for the NFM deviation mode before transmission: "_**Wide**_" uses +-5 kHz deviation for 25 kHz amateur VHF/UHF channels, while "_**Narrow**_" uses +-2.5 kHz deviation for 12.5 kHz PMR/DMR-style channels. The shared [`Biquad`](./src/utils/biquad.h) helper now accepts an explicit pole-pair Q, so higher-order Butterworth cascades can reuse the same biquad coefficient code instead of adding module-specific filter sections.

The FM broadcast transmitter with RDS has been rewritten from the legacy PiFmRds / `libsndfile` stack into a self-contained C++20 [`pifmrds`](./src/pifmrds/) binary. The original implementation mixed C and C++ sources, opened the audio file through `libsndfile`, kept a separate control-pipe path, used `atof` / `strtol`-style argument parsing, installed handlers for all 64 signals, and built the auxiliary `rds_wav` helper. The new version reads 16-bit PCM mono or stereo audio from stdin, parses the WAV header through the shared [`wav_utils`](./src/utils/wav_utils.h) helpers, accepts any supported input sample rate through a reusable [`PolyphaseResampler`](./src/utils/polyphase_resampler.h), validates CLI arguments with shared [`cli_utils`](./src/utils/cli_utils.h) helpers plus module-specific PI / PS / RT checks, and drives `librpitx::ngfmdmasync` directly at the canonical 228 kHz MPX rate. The dedicated [`FmRdsProcessor`](./src/pifmrds/fmrds_processor.h) chain applies a 30 Hz HPF, selectable 50 us / 75 us broadcast pre-emphasis, a cascaded 4th-order 15 kHz Butterworth LPF, joint AGC, and MPX composition: mono audio plus RDS, or stereo `(L+R) + 19 kHz pilot + 38 kHz DSB-SC (L-R) + 57 kHz RDS`, clamped to 75 kHz peak deviation. RDS generation is split into reusable [`RdsEncoder`](./src/pifmrds/rds_encoder.h) and [`RdsModulator`](./src/pifmrds/rds_modulator.h) components: the encoder emits EN 50067 PI / PS / RT / CT groups with CRC and block offset words, while the modulator performs differential encoding, RRC-shaped biphase pulse overlap-add, and phase-locked 57 kHz subcarrier generation. [`easytest.sh`](./easytest.sh) and [`testfmrds.sh`](./scripts/testfmrds.sh) now expose PI, PS, RT, and pre-emphasis settings, while `rds_wav` and the `libsndfile1-dev` system dependency were removed from the build and installation flow.
